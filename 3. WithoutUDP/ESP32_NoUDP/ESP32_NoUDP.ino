...
//(중요!!)socketIoClient의 경우 버전 0.3.0은 esp32를 지원하지 않으므로 more info에 들어가서 GiiHub 파일을 .zip파일로 집적 다운로드하여 라이브러리를 add .zip file기능을 통해 추가
...
const char* hostServer = "143.248.250.30"; // Host Server

SocketIoClient socket;

const long interval = 50; // 데이터를 보낼 주기
...
void event(const char * data, size_t length) {
  ...
  digitalWrite(14, buttonState); // LED 작동
